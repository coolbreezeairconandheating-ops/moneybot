...
3. Every `CHECK_INTERVAL_SECONDS` (default 5 min) during market hours,
   fetches the latest intraday candles for the same contract.
4. Computes EMA(9), EMA(21), ATR(14), and a rolling N-candle high/low.
5. If price breaks the recent high while in an EMA-confirmed uptrend →
   suggests a CE buy. Breaks the recent low in a downtrend → suggests PE.
6. SL/TP1/TP2 are set as ATR multiples from the current price (adapts to
   volatility instead of using fixed point distances).
7. Sends a formatted message to your Telegram chat. Repeated signals in the
